    ...
    return (
        <aside className="flex flex-col h-screen w-[250px] bg-gray-200 px-4 py-6">
            ...
            <nav className="flex flex-col items-center space-y-3 mb-6">
                <Link to="/interview" className="text-blue-600 font-semibold hover:underline">
                    면접
                </Link>
                <Link to="/statistics" className="text-blue-600 font-semibold hover:underline">
                    면접 통계
                </Link>
                <Link to="/settings" className="text-blue-600 font-semibold hover:underline">
                    설정
                </Link>
            </nav>
                    ...
                    <ul className="p-0">
                        {records.map((record) => (
                            ...
                            >
                                {record.summary?.trim()
                                    ? record.summary
                                    : record.question?.slice(0, 20) + '...' || '질문 없음'}
                            </li>
                        ))}
                    </ul>
                )}
